...
Class CatalogHandler extends Db_handler{
  private $x_pag;
  private $all_pages;
  private $all_rows;
  private $tabName;
  ...
  function __construct(){
    parent::__construct();
    $this->tabName = "pokemon";
    $this->setAll($_GET['src']);
  }

  private function setAll($search){
    $this->search = $search;
    $this->all_rows = $this->countRows($this->tabName, ' WHERE identifier LIKE "%'.$this->search.'%"');
  ...
  public function search(){
    ...
    $sth = $this->query('SELECT * FROM '.$this->tabName.' WHERE identifier LIKE "%'.$this->search.'%" LIMIT '.$first.', '.$this->x_pag);
    $tab = [];
...
Class UserHandler extends Db_handler{
  public $tabName;
  public $pswField;
  public $usrField;
  public $emailField;
